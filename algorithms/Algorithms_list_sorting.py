...
class Algorithms (object):
    ...
    def __init__(self, collectionName, collectionNameLocate, checkPointFile, numberNeighbours, algorithmDistance, algorithmChoosePoints, mode, listChangeDiffPercent):
        ...
        self.COLL_RESULT_NAME = self.collName + self.ALGORITHM_CHOOSEPOINTS_NAME[self.choosenAlgorithmChoosePoints] + str(self.numberOfNeighbours) + '_distanceAlgorithm_' +self.ALGORITHM_DISTANCE_NAME[self.choosenAlgorithmDistance] + '_' + 'PERCENT_' + str(self.listChangeDiffPercent).replace('.','-') + '_' + os.path.basename(__file__).split('.')[0]
