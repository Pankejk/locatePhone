...
class Algorithms (object):
    ...
    def __init__(self, collectionName, collectionNameLocate, checkPointFile, numberNeighbours, algorithmDistance, algorithmChoosePoints, mode, numberOfMissingAp):
        ...
        self.mode = mode
        self.numberOfMissingAp = numberOfMissingAp
        ...
        self.COLL_RESULT_NAME = self.collName + self.ALGORITHM_CHOOSEPOINTS_NAME[self.choosenAlgorithmChoosePoints] + str(self.numberOfNeighbours) + '_distanceAlgorithm_' +self.ALGORITHM_DISTANCE_NAME[self.choosenAlgorithmDistance] + '_' + 'MISSING_AP_' + str(self.numberOfMissingAp) + '_' + os.path.basename(__file__).split('.')[0]
...
###################################################################################################
    """methdod loads data from databse and from locate. Then preapres those document
       from fingeprint which has the same AP vector"""
    def loadsDataFingerPrintAndLocate(self):
        ...
        rssiTmpDocs = self.collFingerprint.find({'RSSI_DATA': {'$exists' : True}})
        ...
        rssiTmpDocs = [res for res in rssiTmpDocs]
        magneticLocateDocs = [res for res in magneticLocateDocs]
        rssiLocateDocs = [res for res in rssiLocateDocs]

        
        locateCheckpointApList = []
        for doc in rssiLocateDocs:
            locateCheckpointApList.append(doc['MAC_AP'])

        fingerprintApList = []
        for x in self.x_distinct:
            for y in self.y_distinct:
                fingerprintApList.append({'X': x, 'Y': y, 'AP_LIST': [], 'COUNTER': 0})
  
        for doc in rssiTmpDocs:
            for dic in fingerprintApList:
                if doc['X'] == dic['X'] and doc['Y'] == dic['Y']:
                    dic['AP_LIST'].append(doc['MAC_AP'])
                    
        for dic in fingerprintApList:
            tList = dic['AP_LIST']
            for macFingerprint in tList:
                if macFingerprint in locateCheckpointApList:
                        dic['COUNTER'] += 1
                        
        fingerprintApList = sorted(fingerprintApList, key=lambda x: x['COUNTER'])
            ...
            if anws == 'n':
                ...
                if anws == 'n':
                    ...
                    for doc in self.sumDiff['RSSI']:
                        ...
                        print 'DIFF: ' + str(doc['SUM_DIFF_MEAN'])
...
def main():
    ...
    mode = sys.argv[7]
    numberOfMissingAp = int(sys.argv[8])
    Algorithms(collName, collNameLocate, checkPointFile, numberOfNeighbours, algorithmDistance, algorithmChoosePoints, mode, numberOfMissingAp).startLocate()
